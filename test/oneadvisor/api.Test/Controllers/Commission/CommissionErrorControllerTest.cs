...
namespace api.Test.Controllers.Commission
{
    public class CommissionErrorControllerTest
    {
        ...
        {
            Assert.Equal(11, typeof(OneAdvisor.Model.Commission.Model.CommissionError.CommissionError).PropertyCount());
        ...
        {
            Assert.Equal(6, typeof(CommissionErrorEdit).PropertyCount());
        ...
        public async Task Next()
        {
            ...
            service.Setup(c => c.GetNextError(It.IsAny<ScopeOptions>(), It.Is<Guid>(m => m == error.CommissionStatementId)))
                .Callback((ScopeOptions scopeOptions, Guid a) => options = scopeOptions)
                .ReturnsAsync(error);

            var controller = new CommissionErrorController(service.Object, authService.Object);

            var result = await controller.Next(error.CommissionStatementId);
