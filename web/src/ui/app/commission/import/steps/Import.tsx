...
import {
    commissionImportPreviousStep, commissionImportProgressPercentSelector, commissionImportSelectedColumnsSelector,
    commissionImportSelector, ImportColumn, ImportCommission, importCommission, importCommissionClearResults,
    importCommissionReset, ResultFailure
...
type Props = {
    ...
    resultsSuccess: ImportCommission[];
...
class Import extends Component<Props> {
    ...
    startImport = () => {
        this.props.dispatch(importCommissionClearResults());
        this.props.commissions.forEach(m => {
            this.props.dispatch(importCommission(m));
        });
    };
    ...
    success = () => {
        Modal.success({
            title: 'Success',
            content: 'All commission data has been succesfully imported.'
        });
    };
                        ...
                        <Panel header="Record Data" key="2">
                            {this.props.columns.map(column => {
                                return (
                                    <div key={column.id}>
                                        <b>{column.name}: </b>
                                        {record.importCommission[column.id]}
                                    </div>
                                );
                            })}
                        </Panel>
    ...
    getColumns = () => {
        ...
        const idNumberColumn = getColumn('_id', 'Record', {
            sorter: undefined,
    ...
    reset = () => {
        this.props.dispatch(importCommissionReset());
    };
...
const mapStateToProps = (state: RootState) => {
    ...
    return {
        ...
        progressPercent: commissionImportProgressPercentSelector(state),
