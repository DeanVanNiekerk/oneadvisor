...
export const receivePolicy = (policy: PolicyEdit | null): PolicyReceiveAction => ({
    type: "POLICIES_POLICY_RECEIVE",
    payload: policy,
});

export const getPolicy = (policyId: string, onSuccess: ApiOnSuccess): ApiAction => ({
    type: "API",
...
): ThunkAction<void, RootState, {}, PolicyReceiveAction | ApiAction> => {
...
export const confirmCancelPolicy = (
    showConfirm: ShowConfirm,
    onCancelled: () => void
): ThunkAction<void, RootState, {}, PolicyReceiveAction> => {
    return (dispatch, getState) => {
